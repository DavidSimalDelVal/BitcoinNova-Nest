...
import (
	...
	"BitcoinNova-Nest/walletdmanager"
	"database/sql"
	"encoding/json"
	"io"
	"io/ioutil"
...
var (
	...
	qmlBridge                   *QmlBridge
	transfers                   []bitcoinnovawalletdrpcgo.Transfer
	remoteNodes                 []node
	...
	stringBackupKeys            = ""
	rateUSDBTN                 float64 // USD value for 1 BTN
...
type QmlBridge struct {
	...
	_ func(amountBTN string) string `slot:"getTransferAmountUSD"`
...
func main() {
	...
	if isPlatformDarwin {
		...
		pathToAppFolder := pathToHomeDir + "/Library/Application Support/BitcoinNova-Nest"
	...
	walletdmanager.GracefullyQuitBitcoinnovad()
}
...
func connectQMLToGOFunctions() {
	...
	qmlBridge.ConnectGetTransferAmountUSD(func(amountBTN string) string {
		return amountStringUSDToBTN(amountBTN)
	})
...
func getAndDisplayBalances() {
	...
	if err == nil {
		...
		balanceUSD := walletTotalBalance * rateUSDBTN
	...
	if err == nil {
		...
		if needFullUpdate {
			...
			for index, transfer := range transfers {
				...
				amountString += " BTN (fee: " + strconv.FormatFloat(transfer.Fee, 'f', 2, 64) + ")"
...
func transfer(transferAddress string, transferAmount string, transferPaymentID string, transferFee string) {
	...
	qmlBridge.FinishedSendingTransaction()
	qmlBridge.DisplayPopup("BTNs sent successfully", 4000)
}
...
func startWalletWithWalletInfo(pathToWallet string, passwordWallet string) bool {
	...
	if err != nil {
		log.Warn("error starting Bitcoinnova-service with provided wallet info. error: ", err)
		qmlBridge.FinishedLoadingWalletd()
		qmlBridge.DisplayErrorDialog("Error opening wallet.", err.Error())
		return false
	}

	log.Info("success starting Bitcoinnova-service")
...
func requestRateBTN() {
	response, err := http.Get(urlCryptoCompareBTN)
	...
	if err != nil {
		...
		if err != nil {
			...
			if err := json.Unmarshal(b, &resultInterface); err != nil {
				...
				rateUSDBTN = resultsMap["USD"].(float64)
			}
		}
	}
}
...
func amountStringUSDToBTN(amountBTNString string) string {
	amountBTN, err := strconv.ParseFloat(amountBTNString, 64)
	if err != nil || amountBTN <= 0 || rateUSDBTN == 0 {
		return ""
	}
	amountUSD := amountBTN * rateUSDBTN
